            ...
            <tbody className="divide-y divide-[#E8EAF0]">
              {usdtItems.map((item) => (
                <tr key={item._id || item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-xs font-extrabold text-slate-900">{item.reason}</td>
                  <td className="px-4 py-3.5 text-xs text-slate-500">{item.description}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-rose-600">{Number(item.amount || 0).toLocaleString()}</td>
                  ...
                  <td className="px-4 py-3.5 text-xs text-right space-x-2">
                    <button type="button" onClick={() => onEdit(item)} className="p-1 text-slate-400 hover:text-indigo-600 rounded-md" aria-label="Edit expense">
                      <Pencil className="w-4 h-4 inline" />
                    </button>
                    <button type="button" onClick={() => onDelete(item._id || item.id)} className="p-1 text-slate-400 hover:text-rose-600 rounded-md" aria-label="Delete expense">
