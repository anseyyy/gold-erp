...
export default function BuyPage() {
  ...
  const loadBuys = useCallback(async () => {
    try {
      const data = await podiyanaApi.getBuy();
      setItems(data.items || []);
      setApiError('');
    } catch (error) {
      setApiError(error.response?.data?.message || 'Unable to load Podiyana buy orders.');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    podiyanaApi
      ...
      .catch((error) => {
        if (active)
          setApiError(
            error.response?.data?.message || 'Unable to load Podiyana buy orders.'
          );
      })
      .finally(() => {
  ...
  const submitBuy = async (event) => {
    ...
    try {
      ...
      await podiyanaApi.createBuy(payload);
      await loadBuys();
      setScrap('');
      setTouch('');
      setPure('');
      ...
      setApiError(err.response?.data?.message || 'Failed to create Podiyana buy order.');
    } finally {
  ...
  const deleteBuy = async (id) => {
    await podiyanaApi.deleteBuy(id);
    await loadBuys();
  };
