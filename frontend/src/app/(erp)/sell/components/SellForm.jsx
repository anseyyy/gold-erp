        ...
        <div className="flex items-center gap-20 justify-between w-full">
          <div>
            <h3 className="font-extrabold text-sm text-slate-800 dark:text-slate-100">
              {isEditing ? 'Edit General Sell Order' : 'Record General Sell Order'}
            </h3>
            <p className="text-xs text-slate-400 dark:text-slate-500">
              Complete customer gold trade entry
            </p>
          </div>
    ...
    >
      <form onSubmit={onSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Input
            label={`Received Amount (${payment})`}
            type="number"
            step="any"
            placeholder={`Defaults to total value (${payment === 'USDT' ? '$' : 'Rp'})`}
        ...
        <div className="bg-emerald-50/60 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-800/60 p-4 rounded-xl space-y-2">
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 text-xs">
            ...
            <div>
              <span className="text-slate-400 dark:text-slate-400 font-medium block">Received Amount ({payment}):</span>
              <span className="font-mono font-extrabold text-sky-600 dark:text-sky-400 text-sm">
                {payment === 'USDT' ? formatUSDT(effectiveReceived) : formatIDR(effectiveReceived)}
              </span>
            </div>
            <div>
              <span className="text-slate-400 dark:text-slate-400 font-medium block">Balance To Get ({payment}):</span>
              <span className={`font-mono font-extrabold text-sm ${remainingBalance > 0 ? 'text-amber-600 dark:text-amber-400 font-black' : 'text-emerald-600 dark:text-emerald-400'}`}>
        ...
        <div className="flex justify-end pt-1">
          ...
          >
            {isEditing ? 'Update Sell Order' : 'Record Sell Order'}
          </Button>
        </div>
      </form>
    </Card>
