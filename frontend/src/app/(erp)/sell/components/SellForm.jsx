...
export default function SellForm({
    ...
    return (
        <Card header={<div className="flex items-center justify-between w-full"><div><h3 className="font-extrabold text-sm text-slate-800">{isEditing ? 'Edit General Sell Order' : 'Record General Sell Order'}</h3><p className="text-xs text-slate-400">Complete customer gold trade entry</p></div><div className="flex items-center gap-2">{isEditing && <Button type="button" variant="outline" size="sm" onClick={onCancelEdit}>Cancel</Button>}<Button type="button" variant="outline" size="sm" icon={UserPlus} onClick={onAddCustomer}>+ Add Customer</Button></div></div>}>
            <form onSubmit={onSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Input label="Date" type="date" value={date} onChange={(e) => setDate(e.target.value)} error={errors.date} />
                    <Select label="Customer" value={selectedCustomer} onChange={(e) => setSelectedCustomer(e.target.value)} options={customers.map((c) => ({ value: c, label: c }))} />
                ...
                <div className="bg-indigo-50/60 border border-indigo-100 p-4 rounded-xl"><div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs"><div><span className="text-slate-400 font-medium block">Pure Gold Weight:</span><span className="font-mono font-extrabold text-amber-700 text-sm">{(calculatedPure || 0).toFixed(3)} g</span></div><div><span className="text-slate-400 font-medium block">Total IDR:</span><span className="font-mono font-extrabold text-indigo-700 text-sm">{formatIDR(calculatedTotalIdr)}</span></div><div><span className="text-slate-400 font-medium block">Total Dollar Out:</span><span className="font-mono font-extrabold text-emerald-600 text-sm">{formatUSDT(calculatedTotalDollar)}</span></div><div><span className="text-slate-400 font-medium block">Balance Due ({payment}):</span><span className="font-mono font-extrabold text-slate-800 text-sm">{payment === 'USDT' ? formatUSDT(calculatedBalance) : formatIDR(calculatedBalance)}</span></div></div></div>
